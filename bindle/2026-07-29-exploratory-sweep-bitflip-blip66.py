...
def delimit_intro(mo):
    mo.md("""
    # Exploratory sweep, bitflip blips, blip_freq=0.66 (2026-07-29): double-descent analysis

    Downloads the collated timeseries parquet file for the
    `2026-07-29-exploratory-sweep-bitflip-blip66` SLURM batch job (see
    `slurm/2026-07-29/`) from OSF, then -- for each swept condition
    (L1/L2 regularization mix and environment schedule mode) -- renders a
    compound plot summarizing training/testing dynamics, phenotype
    composition, and double descent across model size `v` and training
    time.

    Like `2026-07-29-exploratory-reseed-bitflip-blip50` and its
    `-l1-9966` sibling, `blip_freq` and `blip_mode` are held fixed for
    every replicate here (`blip_freq=0.66`, `blip_mode=bitflip`) -- a
    `blip_freq` value no prior sweep in this project had tried. Unlike
    those two siblings, `zero_init` is *also* fixed here (`True` only,
    not swept), and `schedule_mode` drops `global` (`{none, local}`
    only). `l1_scale`/`l2_scale` sweeps 7 complementary mixes
    (`l2_scale = 1 - l1_scale`): `{(0.995, 0.005), (0.933, 0.067),
    (0.9925, 0.0075), (0.99, 0.01), (0.985, 0.015), (0.996, 0.004),
    (1.0, 0.0)}`. `v`/`seed` keep the uneven split used by the `-blip50`
    family (`v=0` at 1 replicate, seed 5 only; `v` in
    `{4, 8, 12, 16, 20}`), but at 3 replicates each (seeds 5-7) rather
    than 5, for 224 total replicates across 14 (`l1_scale`/`l2_scale`
    mix, `schedule_mode`) conditions.
    """)
    return
...
@app.cell
def osf_slugs():
    # https://osf.io/2d9c3 -- 2026-07-29-exploratory-sweep-bitflip-blip66
    #   collated timeseries (blip_freq fixed to 0.66, blip_mode fixed to
    #   "bitflip", zero_init fixed to True; l1_scale/l2_scale mix and
    #   schedule_mode swept)
    OSF_SLUGS = {
        "exploratory-sweep-bitflip-blip66": "2d9c3",
    }
    return (OSF_SLUGS,)
...
def delimit_prep(mo):
    ...
    x-axis) and `seed` (the replicate axis). `blip_freq` (`0.66`),
    `blip_mode` (`bitflip`), and `zero_init` (`True`) are all fixed for
    every row in this dataset, so only `l1_scale`/`l2_scale` and
    `schedule_mode` actually vary across conditions here (contrast the
    `2026-07-29-exploratory-reseed-bitflip-blip50` notebooks, which also
    sweep `zero_init`).
    """)
    return
