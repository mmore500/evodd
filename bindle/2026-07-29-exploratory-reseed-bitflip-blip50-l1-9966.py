...
def delimit_intro(mo):
    mo.md("""
    # Exploratory reseed, bitflip blips, blip_freq=0.5, L1 scale zoom around 0.9966 (2026-07-29): double-descent analysis

    Downloads the collated timeseries parquet file for the
    `2026-07-29-exploratory-reseed-bitflip-blip50-l1-9966` SLURM batch job
    (see `slurm/2026-07-29/`) from OSF, then -- for each swept condition
    ...
    Like `2026-07-29-exploratory-reseed-bitflip-blip50`, `blip_freq` and
    `blip_mode` are held fixed at `0.5`/`bitflip` (not swept) for every
    replicate, reproducing (reseeding, against current `HEAD`) the single
    ...
    output, `osf.io/j3frh`). Where `-blip50`'s own sweep tried 3 widely
    spaced (`l1_scale`, `l2_scale`) mixes, this job instead zooms in
    tightly around that artifact's own `l1_scale=0.9966`: 3
    properly-complementary mixes (`l2_scale = 1 - l1_scale`) bracketing it
    -- `(0.9966, 0.0034)`, `(0.996, 0.004)`, `(0.997, 0.003)` -- plus a 4th
    mix, `(0.9966, 0.003)`, the original artifact's own (non-complementary)
    mix verbatim, included for direct comparison against the
    properly-complementary `0.9966`/`0.0034` mix. `zero_init` in
    `{True, False}` and `schedule_mode` in `{none, local, global}` are
    swept as before. `v`/`seed` keep the uneven split used by the original
    randomblip job (`v=0` at 1 replicate, seed 5 only; `v` in
    `{4, 8, 12, 16, 20}` at 5 replicates each, seeds 5-9), for 624 total
    replicates across 24 (`l1_scale`/`l2_scale` mix, `zero_init`,
    `schedule_mode`) conditions.
    """)
    return
...
@app.cell
def osf_slugs():
    # https://osf.io/hmsju -- 2026-07-29-exploratory-reseed-bitflip-blip50-l1-9966
    ...
    OSF_SLUGS = {
        "exploratory-reseed-bitflip-blip50-l1-9966": "hmsju",
    }
    return (OSF_SLUGS,)
