...
# below, which submits 8 explicit array TASK indices (ARRAY_INDICES)
...
# fastest-varying index, exactly matching CHUNK). So picking a task index
# automatically exercises all 3 schedule_mode values (none, local, global)
# for that condition -- this test doubles as a check that concurrently
# running all 3 modes on the 3 CPUs of one job doesn't clobber each
# other's output files (each replicate writes to its own
# ${JOBDIR}/r<gid>/dd_trial_outputs/, keyed by its own global index, so
# same-job concurrency is exactly as isolated as cross-job concurrency).
#
# The 8 selected task indices were picked (via the SAME decomposition
# used in production) to land on v in {0, 8, 16, 20}, one CHUNK=3 triple
# (i.e. one replicate under each of none/local/global) each; blip_freq/
# l1-l2-mix/seed are whatever the decomposition naturally assigns at that
# task (arbitrary, per request -- not swept here). 8 tasks * 3 replicates
# = 24 replicates total:
...
echo "ARRAY_INDICES=${ARRAY_INDICES} (only these 8 of ${N_ARRAY_TASKS} array indices are actually submitted; 8*3=24 replicates)"
