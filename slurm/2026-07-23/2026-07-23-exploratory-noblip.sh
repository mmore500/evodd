...
# Variant of 2026-07-23-exploratory-sweep.sh with blip_freq fixed at 0
# (no blip environments -- purely the 3 unblipped training patterns) instead
# of swept, so blip_counts collapses to [1200,1200,1200,0,0,0] for every
# replicate (see build_schedule in the notebook: blip_freq=0 gives
# _blip_ct=0, a fully-supported degenerate case of weighted_interleave).
#
# Sweep: the single-trial elastic-net GRN notebook (v1..v20 double-descent
# model) across:
#   - blip_freq     fixed at 0 (no blip environments)                (1)
...
# total = 1 * 3 * 2 * 3 * (1*1 + 10*4) = 1 * 3 * 2 * 3 * 41 = 738 replicates.
#
...
# this divides 738 replicates evenly into 738 / 3 = 246 array tasks.
#
...
# N_TASKS_V0 (18) is itself a multiple of CHUNK=3, so no CHUNK-triple
