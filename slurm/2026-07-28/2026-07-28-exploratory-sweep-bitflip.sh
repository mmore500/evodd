...
# structure, index decomposition, and CHUNK-packing scheme, but every
# replicate runs with the notebook's --blip-mode fixed to "bitflip"
# (rather than the default "fixed" hand-picked single-bit flip) -- each
# blip period presents a per-replicate-fixed, uniformly-random single-bit
# flip of its training pattern (see make_blips in the notebook's
# build_schedule), rejecting draws that would coincide with an actual
# training pattern or its sign-negation. blip_freq=0 is dropped from this
# sweep (blip_mode has no effect with no blips at all -- that no-blip
# control is already covered by the base sweep), as is blip_mode=random
# (dropped entirely; this sweep only ever exercises "bitflip"). The
# notebook's classify_by_phenotype_output_masked now measures a "bitflip"
# phenotype-outcome category spanning all 48 possible single-bit
# transformations of the 3 training patterns (not just the one variant
# actually presented in a given replicate), weighted by their expected
# 1-in-16-per-item sampling frequency under make_blips' uniform draw.
#
# Sweep: the single-trial elastic-net GRN notebook (v1..v20 double-descent
# model) across:
#   - blip_freq     in {0.33, 0.4, 0.45, 0.5}                         (4)
...
# total = 4 * 4 * 2 * 2 * (1*1 + 5*4) = 4 * 4 * 2 * 2 * 21 = 1344 replicates,
# i.e. 672 (blip_freq, l1/l2 mix, zero_init, v, seed) CONDITIONS, each run
...
# this divides 1344 replicates evenly into 1344 / 2 = 672 array tasks.
#
...
# N_TASKS_V0 (64) is itself a multiple of CHUNK=2, so no CHUNK-pair
...
run_replicate() {
    ...
    echo "  [gid=\${gid}] blip_freq=\${blip} blip_mode=bitflip seed=\${seed} l1=\${l1} l2=\${l2} v=\${v} zero_init=\${zeroinit} schedule_mode=\${schedulemode} repdir=\${repdir}"
