...
# VARIANT of slurm/2026-07-27/2026-07-27-exploratory-sweep.sh: same v/seed
# structure and CHUNK-packing scheme as that sweep, plus one new swept
# dimension -- blip_mode -- exercising the notebook's new --blip-mode flag
# (see build_schedule in bindle/2026-07-23-exploratory.py): instead of the
# 3 blip periods always presenting a fixed, hand-picked single-bit-flip
# variant of S1/S2/S3, each replicate here fixes (for its whole run) either
# a random single-bit flip of each of S1/S2/S3 ("bitflip") or an entirely
# random +-1 phenotype for each blip slot ("random") -- in both cases,
# re-drawn if the result would coincide with an actual training pattern or
# its sign-negation. blip_freq=0 is dropped from this sweep (blip_mode has
# no effect when there are no blip presentations at all -- that no-blip
# control is already covered by the base sweep).
#
# Sweep: the single-trial elastic-net GRN notebook (v1..v20 double-descent
# model) across:
#   - blip_freq     in {0.1, 0.2, 0.33, 0.5}                         (4)
#   - blip_mode     in {bitflip, random}                             (2)
#     the notebook's --blip-mode flag; see above.
...
#     sweep. Controls how ties are broken in the environment-presentation
...
# total = 4 * 2 * 4 * 2 * 2 * (1*1 + 5*4) = 4*2*4*2*2 * 21 = 2688 replicates,
# i.e. 1344 (blip_freq, blip_mode, l1/l2 mix, zero_init, v, seed)
# CONDITIONS, each run under both schedule_mode values.
#
...
#
# The cluster caps a job array at 1000 queued tasks, so we pack CHUNK=2
# replicates into each array task and run those 2 *concurrently* (one CPU
...
# concurrent replicates are the SAME (blip_freq, blip_mode, l1/l2 mix,
# zero_init, v, seed) condition run under both schedule_mode values side by
# side -- this divides 2688 replicates evenly into 2688 / 2 = 1344 array
# tasks.
#
...
# aligns with CHUNK, with blip_mode_idx added as the new SLOWEST-varying
# (outermost) dimension relative to the base sweep:
...
#     blip_idx = (r / N_SCHEDULE / N_ZERO / N_MIX) % N_BLIP;
#     blipmode_idx = r / N_SCHEDULE / N_ZERO / N_MIX / N_BLIP.
...
#     blip_idx = (r' / N_SCHEDULE / N_ZERO / N_V_REST / N_MIX / N_REST_SEED) % N_BLIP;
#     blipmode_idx = r' / N_SCHEDULE / N_ZERO / N_V_REST / N_MIX / N_REST_SEED / N_BLIP.
# N_TASKS_V0 (128) is itself a multiple of CHUNK=2, so no CHUNK-pair
...
N_V_REST=${#V_REST[@]}
N_TASKS_V0=$((N_BLIP * N_BLIPMODE * N_MIX * N_ZERO * N_SCHEDULE * N_V0_SEED))
N_TASKS_REST=$((N_BLIP * N_BLIPMODE * N_MIX * N_ZERO * N_SCHEDULE * N_REST_SEED * N_V_REST))
N_TASKS=$((N_TASKS_V0 + N_TASKS_REST))
CHUNK=2
N_ARRAY_TASKS=$(((N_TASKS + CHUNK - 1) / CHUNK))
NUM_EPOCH=138889
echo "N_BLIP=${N_BLIP} BLIP_FREQS=${BLIP_FREQS[*]}"
echo "N_BLIPMODE=${N_BLIPMODE} BLIP_MODES=${BLIP_MODES[*]}"
...
run_replicate() {
    ...
    if [ "\${gid}" -lt "${N_TASKS_V0}" ]; then
        ...
        local rem4=\$((rem3 / ${N_BLIP}))
        local blipmode_idx=\$((rem4 % ${N_BLIPMODE}))
        v=0
        seed="\${V0_SEEDS[0]}"
    else
        ...
        local rem5=\$((rem4 / ${N_REST_SEED}))
        local blip_idx=\$((rem5 % ${N_BLIP}))
        local rem6=\$((rem5 / ${N_BLIP}))
        local blipmode_idx=\$((rem6 % ${N_BLIPMODE}))
        v="\${V_REST[\${v_idx}]}"
        seed="\${REST_SEEDS[\${seed_idx}]}"
    fi

    local blip="\${BLIP_FREQS[\${blip_idx}]}"
    local blipmode="\${BLIP_MODES[\${blipmode_idx}]}"
    ...
    echo "  [gid=\${gid}] blip_freq=\${blip} blip_mode=\${blipmode} seed=\${seed} l1=\${l1} l2=\${l2} v=\${v} zero_init=\${zeroinit} schedule_mode=\${schedulemode} repdir=\${repdir}"
...
# columns stamped by the notebook's run cell) under r<gid>/dd_trial_outputs/, so a
